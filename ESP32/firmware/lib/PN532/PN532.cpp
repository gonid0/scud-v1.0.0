...
bool PN532::setRfTimings(uint8_t fATR_RES, uint8_t fRetry)
{
    pn532_packetbuffer[0] = PN532_COMMAND_RFCONFIGURATION;
    pn532_packetbuffer[1] = 0x02;    // CfgItem: Various timings
    pn532_packetbuffer[2] = 0x00;    // RFU
    pn532_packetbuffer[3] = fATR_RES;
    pn532_packetbuffer[4] = fRetry;

    if (HAL(writeCommand)(pn532_packetbuffer, 5))
        return false;

    // RFConfiguration отвечает только status-байтом (никакого payload),
    // поэтому readResponse возвращает 0 при успехе. Считаем ок всё, что >=0.
...
bool PN532::inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
    ...
    int16_t status = HAL(readResponse)(response, *responseLength, 4000);
    if (status < 0) {
        Serial.printf("[PN532] inDataExchange: UART timeout/error (%d)\n", status);
        return false;
    }

    if ((response[0] & 0x3f) != 0) {
        Serial.printf("[PN532] inDataExchange: status=0x%02X\n", response[0]);
        return false;
    }
